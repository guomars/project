...
struct ROLEEXTPROP_ATK
{
	...
	struct	ADDON_DAMAGE//	物理攻击附加魔法伤害
	{
		int damage_low;
		int damage_high;

	};
	ADDON_DAMAGE addon_damage[NUM_MAGICCLASS];
...
//	Set extend state
inline void glb_SetExtState(unsigned int* p, int n)
{
	p[n >> 5] |= (1 << (n & 31));
}

//	Remove extend state
inline void glb_RemExtState(unsigned int* p, int n)
{
	p[n >> 5] &= ~(1 << (n & 31));
}

//	Get extend state
inline bool glb_GetExtState(const unsigned int* p, int n)
{
