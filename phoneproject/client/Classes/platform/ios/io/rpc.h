	...
	public:
		...
		void Update()
		{
			std::vector<Rpc *> tmp;
			{
				...
				//MapEraser<Map> e(map);
                std::vector<Map::iterator> IteratorVector;
				for(Map::iterator it = map.begin(), ie = map.end(); it != ie; ++it)
				{
					...
					{
						tmp.push_back(rpc);
						IteratorVector.push_back(it);
					}
                    map.erase(it);
				}
                for(std::vector<Map::iterator>::iterator it = IteratorVector.begin(), ie = IteratorVector.end(); it != ie; ++it)
                {
                    map.erase(*it);
                }
			}
